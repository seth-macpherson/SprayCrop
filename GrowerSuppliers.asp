...
REM KILROY
	Response.Redirect("GrowerSuppliers.asp")
END IF

'Initialize Form Fields
DIM formSupplier, formLicenseNo, formActive, lGrowerID
formSupplier = Request.Form.Item("supplier")
...
'	Response.Redirect("GrowerSuppliers.asp?GrowerID=" & formGrowerID)
'END IF 
IF Request.QueryString("task") = "activate" and urlID <> "" THEN
	conn.execute "exec growersupplier$activate " & urlid & ", 1"
	Response.Redirect("GrowerSuppliers.asp?GrowerID=" & formGrowerID)
END IF 
IF Request.QueryString("task") = "deactivate" and urlID <> "" THEN
	conn.execute "exec growersupplier$activate " & urlid & ", 0"
	Response.Redirect("GrowerSuppliers.asp?GrowerID=" & formGrowerID)
END IF
...
IF Request.Form.Item("insert") <> "" OR Request.Form.Item("update") <> "" OR Request.Form.Item("f_action") <> "" THEN
	...
	'Form Validation
	IF NOT ValidateDatatype(Request.Form.Item("supplier"), "nvarchar", "Supplier", FALSE) THEN
		errorFound = TRUE
...
'	IF NOT errorFound AND Request.Form.Item("update") <> "" THEN  
	IF NOT errorFound AND (Request.Form("update") <> "" OR Request.Form.Item("f_action") = "UPDATE") THEN  
		conn.execute "exec growersupplier$upd " & formid & ", '" & formsupplier & "'"
		EndConnect(conn)
		lGrowerID = Request.Form.item("HF_Growerid")
		Response.Redirect("GrowerSuppliers.asp?GrowerID=" & lGrowerID)
	END IF 
	'INSERT
...
'Response.Flush
		dim newRecID
		lGrowerID = Request.Form.item("HF_Growerid")
		conn.execute "exec growersupplier$add " & session("growerid") & ", '" & formsupplier & "'"
'		EndConnect(conn)
		'if newRecID = 0 then
		'	Response.Redirect("GrowerSuppliers.asp?GrowerID=" & lGrowerID & "&Err=DUP")
		'else
			Response.Redirect("GrowerSuppliers.asp?GrowerID=" & lGrowerID)
		'end if
	END IF 'insert	
...
IF formID <> 0 and not errorFound THEN
	set rsEditGrowerLocation = conn.execute("exec growersupplier$list " & session("growerid") & "," & formid)
	IF NOT rsEditGrowerLocation.eof THEN
		formSupplier = rsEditGrowerLocation.Fields("supplier")
...
<h1> > Grower Suppliers</h1><br><img src="images/spacer.gif" height="4" width="1" border="0">
</td></tr></table>
...
<tr bgcolor=#dddddd>
	<td valign="top">&nbsp;</td>
	<td valign="top"><h2>Supplier</h2></td>
	<td valign="top"><h2>&nbsp;</h2></td>

</tr>
<%
	dim i, rsGrowerLocs
	set rsGrowerLocs = conn.execute("exec growersupplier$list " & session("growerid")) 
...
<tr>
	...
	<td class="bodytext" valign="top"><%=rsGrowerLocs.Fields("Supplier")%>&nbsp;</td>
	<td valign="top" class="bodytext" bgcolor=#eeeeee><a href="GrowerSuppliers.asp?ID=<%=rsGrowerLocs.Fields("supplierid")%>&GrowerID=<%=formGrowerID%>#edit" class="bodytext">Edit</a> | <% IF rsGrowerLocs.Fields("active") = 0 then%><a href="GrowerSuppliers.asp?task=activate&ID=<%=rsGrowerLocs.Fields("supplierid")%>&GrowerID=<%=formGrowerID%>" onclick="javascript: return confirm('Are you sure you want to activate this record?');">Make Active</a><%else%><a href="GrowerSuppliers.asp?task=deactivate&ID=<%=rsGrowerLocs.Fields("supplierid")%>&GrowerID=<%=formGrowerID%>" onclick="javascript: return confirm('Are you sure you want to DeActivate this record?');">Make InActive</a><%end if%></td>
...
<a name="edit"></a>
<form action="GrowerSuppliers.asp" method="post">
...
<tr>
	<td valign="top" align="right">
		<span class="subtitle"><label for="supplier"><% IF  urlID <> 0 THEN%>Update<% ELSE %>Add<% END IF %> Supplier</label>:</span>
	</td>
	<td valign="top">
		<span class="bodytext"><input type="text" value="<%=formSupplier%>" name="supplier"  class="bodytext" size="25" maxlength="150"></span>
	</td>
</tr>
