...
REM KILROY
	Response.Redirect("GrowerReferrers.asp")
END IF

'Initialize Form Fields
DIM formReferrer, formLicenseNo, formActive, lGrowerID
formReferrer = Request.Form.Item("referrer")
...
'	Response.Redirect("GrowerReferrers.asp?GrowerID=" & formGrowerID)
'END IF 
IF Request.QueryString("task") = "activate" and urlID <> "" THEN
	conn.execute "exec growerreferrer$activate " & urlid & ", 1"
	Response.Redirect("GrowerReferrers.asp?GrowerID=" & formGrowerID)
END IF 
IF Request.QueryString("task") = "deactivate" and urlID <> "" THEN
	conn.execute "exec growerreferrer$activate " & urlid & ", 0"
	Response.Redirect("GrowerReferrers.asp?GrowerID=" & formGrowerID)
END IF
...
IF Request.Form.Item("insert") <> "" OR Request.Form.Item("update") <> "" OR Request.Form.Item("f_action") <> "" THEN
	...
	'Form Validation
	IF NOT ValidateDatatype(Request.Form.Item("referrer"), "nvarchar", "Referrer", FALSE) THEN
		errorFound = TRUE
...
'	IF NOT errorFound AND Request.Form.Item("update") <> "" THEN  
	IF NOT errorFound AND (Request.Form("update") <> "" OR Request.Form.Item("f_action") = "UPDATE") THEN  
		conn.execute "exec growerreferrer$upd " & formid & ", '" & formreferrer & "'"
		EndConnect(conn)
		lGrowerID = Request.Form.item("HF_Growerid")
		Response.Redirect("GrowerReferrers.asp?GrowerID=" & lGrowerID)
	END IF 
	'INSERT
...
'Response.Flush
		dim newRecID
		lGrowerID = Request.Form.item("HF_Growerid")
		conn.execute "exec growerreferrer$add " & session("growerid") & ", '" & formreferrer & "'"
'		EndConnect(conn)
		'if newRecID = 0 then
		'	Response.Redirect("GrowerReferrers.asp?GrowerID=" & lGrowerID & "&Err=DUP")
		'else
			Response.Redirect("GrowerReferrers.asp?GrowerID=" & lGrowerID)
		'end if
	END IF 'insert	
...
IF formID <> 0 and not errorFound THEN
	set rsEditGrowerLocation = conn.execute("exec growerreferrer$list " & session("growerid") & "," & formid)
	IF NOT rsEditGrowerLocation.eof THEN
		formReferrer = rsEditGrowerLocation.Fields("referrer")
...
<h1> > Grower Referrers</h1><br><img src="images/spacer.gif" height="4" width="1" border="0">
</td></tr></table>
...
<tr bgcolor=#dddddd>
	<td valign="top">&nbsp;</td>
	<td valign="top"><h2>Referral</h2></td>
	<td valign="top"><h2>&nbsp;</h2></td>

</tr>
<%
	dim i, rsGrowerLocs
	set rsGrowerLocs = conn.execute("exec growerreferrer$list " & session("growerid")) 
...
<tr>
	...
	<td class="bodytext" valign="top"><%=rsGrowerLocs.Fields("Referrer")%>&nbsp;</td>
	<td valign="top" class="bodytext" bgcolor=#eeeeee><a href="GrowerReferrers.asp?ID=<%=rsGrowerLocs.Fields("referrerid")%>&GrowerID=<%=formGrowerID%>#edit" class="bodytext">Edit</a> | <% IF rsGrowerLocs.Fields("active") = 0 then%><a href="GrowerReferrers.asp?task=activate&ID=<%=rsGrowerLocs.Fields("referrerid")%>&GrowerID=<%=formGrowerID%>" onclick="javascript: return confirm('Are you sure you want to activate this record?');">Make Active</a><%else%><a href="GrowerReferrers.asp?task=deactivate&ID=<%=rsGrowerLocs.Fields("referrerid")%>&GrowerID=<%=formGrowerID%>" onclick="javascript: return confirm('Are you sure you want to DeActivate this record?');">Make InActive</a><%end if%></td>
...
<a name="edit"></a>
<form action="GrowerReferrers.asp" method="post">
...
<tr>
	<td valign="top" align="right">
		<span class="subtitle"><label for="referrer"><% IF  urlID <> 0 THEN%>Update<% ELSE %>Add<% END IF %> Referrer</label>:</span>
	</td>
	<td valign="top">
		<span class="bodytext"><input type="text" value="<%=formReferrer%>" name="referrer"  class="bodytext" size="25" maxlength="150"></span>
	</td>
</tr>
