...
DIM formLocation, formWatershed, formActive, lGrowerID
formLocation = Request.Form.Item("Location")
formWatershed = Request.Form.Item("Watershed")
...
IF Request.QueryString("task") = "activate" and urlID <> "" THEN
	ActivateGrowersLocation(urlID)
	Response.Redirect("GrowerLocations.asp?GrowerID=" & formGrowerID)
END IF 
IF Request.QueryString("task") = "deactivate" and urlID <> "" THEN
	DeActivateGrowersLocation(urlID)
	Response.Redirect("GrowerLocations.asp?GrowerID=" & formGrowerID)
END IF
...
'	IF NOT errorFound AND Request.Form.Item("update") <> "" THEN  
	IF NOT errorFound AND (Request.Form("update") <> "" OR Request.Form.Item("f_action") = "UPDATE") THEN  
		urlID = UpdateGrowersLocation(formID, formLocation, formWatershed)
		EndConnect(conn)
		lGrowerID = Request.Form.item("HF_Growerid")
		Response.Redirect("GrowerLocations.asp?GrowerID=" & lGrowerID)
	END IF 
	'INSERT
...
'Response.Flush
		dim newRecID
		lGrowerID = Request.Form.item("HF_Growerid")
		newRecID = InsertGrowersLocation(lGrowerID, formLocation, formWatershed)
'		EndConnect(conn)
		if newRecID = 0 then
...
IF formID <> 0 and not errorFound THEN
	set rsEditGrowerLocation = GetGrowersLocationsByID(formID)
	IF NOT rsEditGrowerLocation.eof THEN
		formLocation = rsEditGrowerLocation.Fields("GLoc_Location")
		formWatershed = rsEditGrowerLocation.Fields("GLoc_PURS_Watershed")
		formActive = rsEditGrowerLocation.Fields("GLoc_Active")
	END IF
...
<h1> > Grower Locations</h1><br><img src="images/spacer.gif" height="4" width="1" border="0">
</td></tr></table>
...
<tr bgcolor=#dddddd>
	...
	<td valign="top"><h2>Watershed</h2></td>
	<td valign="top"><h2>&nbsp;</h2></td>
</tr>
<%
	dim i, rsGrowerLocs
	set rsGrowerLocs = GetALLGrowersLocationsByGrowerID(formGrowerID)
...
<tr>
	...
	<td class="bodytext" valign="top"><%=rsGrowerLocs.Fields("GLoc_Location")%>&nbsp;</td>
	<td class="bodytext" valign="top"><%=rsGrowerLocs.Fields("GLoc_PURS_Watershed")%>&nbsp;</td>
	<td valign="top" class="bodytext" bgcolor=#eeeeee><a href="GrowerLocations.asp?ID=<%=rsGrowerLocs.Fields("GLoc_GrowerLocationID")%>&GrowerID=<%=formGrowerID%>#edit" class="bodytext">Edit</a> | <% IF rsGrowerLocs.Fields("GLoc_Active") = 0 then%><a href="GrowerLocations.asp?task=activate&ID=<%=rsGrowerLocs.Fields("GLoc_GrowerLocationID")%>&GrowerID=<%=formGrowerID%>" onclick="javascript: return confirm('Are you sure you want to activate this record?');">Make Active</a><%else%><a href="GrowerLocations.asp?task=deactivate&ID=<%=rsGrowerLocs.Fields("GLoc_GrowerLocationID")%>&GrowerID=<%=formGrowerID%>" onclick="javascript: return confirm('Are you sure you want to DeActivate this record?');">Make InActive</a><%end if%></td>
...
<a name="edit"></a>
<form action="GrowerLocations.asp" method="post">
...
<tr>
	<td valign="top" align="right">
		<span class="subtitle"><label for="Location"><% IF  urlID <> 0 THEN%>Update<% ELSE %>Add<% END IF %> Location</label>:</span>
	</td>
	<td valign="top">
		<span class="bodytext"><input type="text" value="<%=formLocation%>" name="Location"  class="bodytext" size="25" maxlength="150"></span>
	</td>
</tr>

<tr>
	<td valign="top" align="right">
		<span class="subtitle"><label for="PURS_Watershed">PURS Watershed</label>:</span>
	</td>
	<td valign="top"><span class="bodytext">
		<select id="PURS_Watershed" name="Watershed" class="bodytext"><option></option>
			<option value="Middle Columbia" <%if trim(formWatershed) = "Middle Columbia" then Response.write("selected")%>>Middle Columbia</option>
			<option value="Deschutes" <%if trim(formWatershed) = "Deschutes" then Response.write("selected")%>>Deschutes</option>
			<option value="Black Rock Desert" <%if trim(formWatershed) = "Black Rock Desert" then Response.write("selected")%>>Black Rock Desert</option>
			<option value="John Day" <%if trim(formWatershed) = "John Day" then Response.write("selected")%>>John Day</option>
			<option value="Klamath" <%if trim(formWatershed) = "Klamath" then Response.write("selected")%>>Klamath</option>
			<option value="Lower Columbia" <%if trim(formWatershed) = "Lower Columbia" then Response.write("selected")%>>Lower Columbia</option>
			<option value="Lower Snake" <%if trim(formWatershed) = "Lower Snake" then Response.write("selected")%>>Lower Snake</option>
			<option value="Middle Snake/Boise" <%if trim(formWatershed) = "Middle Snake/Boise" then Response.write("selected")%>>Middle Snake/Boise</option>
			<option value="Middle Snake Powder" <%if trim(formWatershed) = "Middle Snake Powder" then Response.write("selected")%>>Middle Snake Powder</option>
			<option value="Northern California Coastal" <%if trim(formWatershed) = "Northern California Coastal" then Response.write("selected")%>>Northern California Coastal</option>
			<option value="Northern Oregon Coastal" <%if trim(formWatershed) = "Northern Oregon Coastal" then Response.write("selected")%>>Northern Oregon Coastal</option>
			<option value="Oregon Closed Basins" <%if trim(formWatershed) = "Oregon Closed Basins" then Response.write("selected")%>>Oregon Closed Basins</option>
			<option value="Southern Oregon Coastal" <%if trim(formWatershed) = "Southern Oregon Coastal" then Response.write("selected")%>>Southern Oregon Coastal</option>
			<option value="Upper Sacramento" <%if trim(formWatershed) = "Upper Sacramento" then Response.write("selected")%>>Upper Sacramento</option>
			<option value="Willamette" <%if trim(formWatershed) = "Willamette" then Response.write("selected")%>>Willamette</option>
		</select>
		</span>
	</td>
</tr>
