...
REM KILROY
	Response.Redirect("GrowerSupervisors.asp")
END IF
...
'	Response.Redirect("GrowerSupervisors.asp?GrowerID=" & formGrowerID)
'END IF 
IF Request.QueryString("task") = "activate" and urlID <> "" THEN
	conn.execute "exec growersupervisor$activate " & urlid & ", 1"
	Response.Redirect("GrowerSupervisors.asp?GrowerID=" & formGrowerID)
END IF 
IF Request.QueryString("task") = "deactivate" and urlID <> "" THEN
	conn.execute "exec growersupervisor$activate " & urlid & ", 0"
	Response.Redirect("GrowerSupervisors.asp?GrowerID=" & formGrowerID)
END IF
...
IF Request.Form.Item("insert") <> "" OR Request.Form.Item("update") <> "" OR Request.Form.Item("f_action") <> "" THEN
	...
	'Form Validation
	IF NOT ValidateDatatype(Request.Form.Item("supervisor"), "nvarchar", "Supervisor", FALSE) THEN
		errorFound = TRUE
...
'	IF NOT errorFound AND Request.Form.Item("update") <> "" THEN  
	IF NOT errorFound AND (Request.Form("update") <> "" OR Request.Form.Item("f_action") = "UPDATE") THEN  
		conn.execute "exec growersupervisor$upd " & formid & ", '" & formsupervisor & "', '" & formlicenseno & "'"
		EndConnect(conn)
		lGrowerID = Request.Form.item("HF_Growerid")
		Response.Redirect("GrowerSupervisors.asp?GrowerID=" & lGrowerID)
	END IF 
	'INSERT
...
'Response.Flush
		dim newRecID
		lGrowerID = Request.Form.item("HF_Growerid")
		conn.execute "exec growersupervisor$add " & session("growerid") & ", '" & formsupervisor & "', '" & formlicenseno & "'"
'		EndConnect(conn)
		'if newRecID = 0 then
		'	Response.Redirect("GrowerSupervisors.asp?GrowerID=" & lGrowerID & "&Err=DUP")
		'else
			Response.Redirect("GrowerSupervisors.asp?GrowerID=" & lGrowerID)
		'end if
	END IF 'insert	
...
IF formID <> 0 and not errorFound THEN
	set rsEditGrowerLocation = conn.execute("exec growersupervisor$list " & session("growerid") & "," & formid)
	IF NOT rsEditGrowerLocation.eof THEN
		formSupervisor = rsEditGrowerLocation.Fields("supervisor")
...
<html>
<head>
	<title>Growers Supervisors</title>
...
<h1> > Grower Supervisors</h1><br><img src="images/spacer.gif" height="4" width="1" border="0">
</td></tr></table>
...
<tr bgcolor=#dddddd>
	<td valign="top">&nbsp;</td>
	<td valign="top"><h2>Supervisor</h2></td>
...
<%
	dim i, rsGrowerLocs
	set rsGrowerLocs = conn.execute("exec growersupervisor$list " & session("growerid")) 
...
<tr>
	...
	<td class="bodytext" valign="top"><%=rsGrowerLocs.Fields("Supervisor")%>&nbsp;</td>
	<td class="bodytext" valign="top"><%=rsGrowerLocs.Fields("LicenseNo")%>&nbsp;</td>
	<td valign="top" class="bodytext" bgcolor=#eeeeee><a href="GrowerSupervisors.asp?ID=<%=rsGrowerLocs.Fields("supervisorid")%>&GrowerID=<%=formGrowerID%>#edit" class="bodytext">Edit</a> | <% IF rsGrowerLocs.Fields("active") = 0 then%><a href="GrowerSupervisors.asp?task=activate&ID=<%=rsGrowerLocs.Fields("supervisorid")%>&GrowerID=<%=formGrowerID%>" onclick="javascript: return confirm('Are you sure you want to activate this record?');">Make Active</a><%else%><a href="GrowerSupervisors.asp?task=deactivate&ID=<%=rsGrowerLocs.Fields("supervisorid")%>&GrowerID=<%=formGrowerID%>" onclick="javascript: return confirm('Are you sure you want to DeActivate this record?');">Make InActive</a><%end if%></td>
...
<a name="edit"></a>
<form action="GrowerSupervisors.asp" method="post">
...
<tr>
	<td valign="top" align="right">
		<span class="subtitle"><label for="supervisor"><% IF  urlID <> 0 THEN%>Update<% ELSE %>Add<% END IF %> Supervisor</label>:</span>
	</td>
	<td valign="top">
